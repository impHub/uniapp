<template>
	<view class="container">
		...
		<view v-show="step===0">
			<view class="form-item-group">
				<u-form-item label="图片上传" label-position="top">
					...
					<u-upload ref="uploadOpposite" :action="updateActionOcr" :before-upload="beforeUpload" :size-type="updateSizeType" :header="updateHeader" :form-data="{group: 'resident',type: 'opposite'}" @on-success="oppositeSuccess" @on-remove="uploadRemove(2)" :show-progress="false" upload-text="身份证人像面" :multiple="false" :max-count="1" width="210"></u-upload>
					<u-upload ref="uploadFront" :action="updateActionOcr" :before-upload="beforeUpload" :size-type="updateSizeType" :header="updateHeader" :form-data="{group: 'resident',type: 'front'}" @on-success="frontSuccess" @on-remove="uploadRemove(3)" :show-progress="false" upload-text="身份证国徽面" :multiple="false" :max-count="1" width="210"></u-upload>
				</u-form-item>
			</view>
			<u-cell-group title="基本信息 - 上传图片后自动识别">
				...
				<u-cell-item title="身份证号" :value="form.idCard || '-'" :arrow="false"></u-cell-item>
				...
				<u-cell-item title="身份证有效期起止" :value="[form.validStart,form.validEnd].join(' - ') || '-'" :arrow="false"></u-cell-item>
		...
		<u-select v-model="relationShow" :list="[propertyList, relationList]" mode="mutil-column" @confirm="selectRelation"></u-select>
		<u-select v-model="houseShow" :list="houseList" mode="mutil-column-auto" label-name="name" value-name="id" @confirm="selectHouse"></u-select>
	</view>
</template>
...
<script>
	export default {
		...
		data(){
			...
			return {
				steps: [{name: '基本信息'}, {name:'上传图片'},{name: '选择房屋'},{name: '门禁授权'}],
				step: 0,
		...
		methods:{
			...
			checkStep(step){
				...
				if(step === 0){
					...
					// return true
					return [f.name, f.sex, f.idCard, f.birth, f.nation, f.address, f.validStart, f.validEnd, f.cardOrg, f.avatar, f.cardFront, f.cardOpposite].reduce((x, y) => x&&y&&y.length>0, true)
				}else if(step === 2){
					// return true
					return !!this.houseSelected.length
				}else if(step === 3){		//提交
					return this.checkStep(0) && this.checkStep(1) && this.checkStep(2) && !!f.deviceAddressIds.length
				}
				return false
			},
			beforeUpload(index){
			...
			uploadRemove(index){
				if(index === 2){
					...
					Object.assign(this.form, {name, sex, nation, birth, address, idCard, cardOpposite: imgUrl})
				}else if(index === 3){
					const {cardOrg, validStart, validEnd, imgUrl} = {}
					Object.assign(this.form, {cardOrg, validStart, validEnd, cardFront: imgUrl})
				}
			},
			avatarSuccess(res){
				if(res.code === 0){
			...
			changeHouse(){
				this.form.residentRooms = this.houseSelected.map(d => {
					return {roomId: d.id, property: d.property, relationOwner: d.relation}
				})
				// 选择的房屋变化时，授权单元也要变化
				this.changeQuick()
			},
			changeQuick(){
				...
				this.guardList.map(item => {
					if(item.type === 'front'){
						...
						// 如果是已选择房屋对应的单元门禁
						if(this.houseSelected.findIndex(h => h.buildId===item.buildId && h.unitId===item.unitId) > -1){
			...
			changeGuard(){
				this.form.deviceAddressIds = this.guardList.filter(g=>g.checked===true).map(g=>g.id)
			},
			selectHouse(data){
				...
				if(this.houseSelected.findIndex(item => item.id === house.value) < 0){
					this.houseTemp = {id: house.value, name: house.label, buildId: build.value, buildName: build.label, unitId: unit.value, unitName: unit.label}
